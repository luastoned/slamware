...
export class ProjectionGroupScoresComponent implements OnInit {
  ...
  constructor(private sanitizer: DomSanitizer, private assetService: AssetService, private poetService: PoetService, private groupService: GenericDataService<Group>) {
  ...
  getScore(poet: Poet) {
    ...
    if (group_id === undefined) return 0;

    if (poet.scores == undefined) return 0;
